 ...
 * mystack API Class
 * 
 * Handles API communication with mystack backend
 */
...
class mystack_API {
    ...
    public function __construct() {
        $this->api_url = get_option('mystack_api_url', 'https://api.mystack.ai');
        $this->api_key = get_option('mystack_api_key', '');
        
        // Add REST API endpoints
        add_action('rest_api_init', array($this, 'register_rest_routes'));
        
        // Add webhook handlers
        add_action('wp_ajax_nopriv_mystack_webhook', array($this, 'handle_webhook'));
        add_action('wp_ajax_mystack_webhook', array($this, 'handle_webhook'));
    }
    ...
    public function register_rest_routes() {
        register_rest_route('mystack/v1', '/chat', array(
            'methods' => 'POST',
            'callback' => array($this, 'handle_chat_message'),
            'permission_callback' => '__return_true'
        ));
        
        register_rest_route('mystack/v1', '/ticket', array(
            'methods' => 'POST',
            'callback' => array($this, 'handle_ticket_submission'),
            'permission_callback' => '__return_true'
        ));
        
        register_rest_route('mystack/v1', '/widget-config', array(
            'methods' => 'GET',
    ...
    /**
     * Handle webhook from mystack
     */
    public function handle_webhook() {
        // Verify webhook signature
        $signature = $_SERVER['HTTP_X_mystack_SIGNATURE'] ?? '';
        ...
        switch ($data['event'] ?? '') {
            ...
            case 'chat.message':
                ...
                break;
            default:
                error_log('Unknown mystack webhook event: ' . ($data['event'] ?? 'none'));
        }
    ...
    private function handle_ticket_created($ticket_data) {
        ...
        do_action('mystack_ticket_created', $ticket_data);
    }
    ...
    private function handle_ticket_updated($ticket_data) {
        do_action('mystack_ticket_updated', $ticket_data);
    }
    ...
    private function handle_chat_webhook($chat_data) {
        do_action('mystack_chat_message', $chat_data);
    }
    
    /**
     * Make API request to mystack
     */
    private function make_api_request($method, $endpoint, $data = null) {
        ...
        $args = array(
            ...
            'headers' => array(
                ...
                'User-Agent' => 'mystack-WordPress-Plugin/' . mystack_PLUGIN_VERSION
            ),
            'timeout' => 30
        );
