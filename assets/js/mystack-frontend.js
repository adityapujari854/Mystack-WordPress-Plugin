...
(function($) {
    ...
    // mystack namespace
    window.mystack = window.mystack || {};
    
    // Configuration
    const config = {
        apiUrl: mystack_frontend.api_url || 'https://api.mystack.ai',
        apiKey: mystack_frontend.api_key || '',
        debug: mystack_frontend.debug || false
    };
    ...
    const utils = {
        log: function(message, data) {
            if (config.debug) {
                console.log('[mystack]', message, data || '');
            }
        },
        
        error: function(message, error) {
            console.error('[mystack Error]', message, error || '');
        },
        
        generateId: function() {
            return 'mystack-' + Math.random().toString(36).substr(2, 9);
        },
    ...
    class mystackWidget {
        ...
        createWidget() {
            ...
            this.container = $('<div></div>')
                ...
                .addClass('mystack-widget-container')
                .addClass('mystack-theme-' + this.options.theme)
                .addClass('mystack-position-' + this.options.position)
                .css({
                    position: 'fixed',
                    zIndex: 999999,
                    display: 'none'
            ...
            this.button = $('<button></button>')
                .addClass('mystack-widget-button')
            ...
            const header = $('<div></div>')
                .addClass('mystack-widget-header')
                .css({
                    padding: '16px',
                ...
                .html(`
                    <span class="mystack-widget-title">${utils.sanitizeHtml(this.options.title)}</span>
                    <button class="mystack-widget-close" aria-label="Close chat" style="
                        background: none;
                        border: none;
                        font-size: 20px;
                        cursor: pointer;
        ...
        bindEvents() {
            ...
            // Close button click
            this.container.on('click', '.mystack-widget-close', () => {
                this.close();
            });
    ...
    window.mystack = {
        ...
        TicketForm: mystackTicketForm,
        Chatbot: mystackChatbot,
        
        // Initialize global widget
        initGlobalWidget: function(options) {
            if (window.mystackGlobalWidget) {
                window.mystackGlobalWidget.destroy();
            }
            
            window.mystackGlobalWidget = new mystackWidget(options);
            return window.mystackGlobalWidget;
        },
        
        // Initialize all shortcode elements
        initShortcodes: function() {
            // Initialize chatbot shortcodes
            $('.mystack-chatbot-shortcode').each(function() {
                const $this = $(this);
                const options = {
                ...
                new mystackChatbot(this, options);
            });
            
            // Initialize ticket form shortcodes
            $('.mystack-ticket-form-shortcode').each(function() {
                const $this = $(this);
                const options = {
                ...
                new mystackTicketForm(this, options);
            });
            
            // Initialize widget shortcodes
            $('.mystack-widget-shortcode').each(function() {
                const $this = $(this);
                const options = {
                    chatbotId: $this.data('chatbot-id'),
                    theme: $this.data('theme') || 'light',
    ...
    $(document).ready(function() {
        utils.log('mystack frontend loaded');
        
        // Initialize shortcodes
        window.mystack.initShortcodes();
        
        // Initialize global widget if configured
        if (mystack_frontend.global_widget && mystack_frontend.global_widget.enabled) {
            window.mystack.initGlobalWidget(mystack_frontend.global_widget);
        }
    });
