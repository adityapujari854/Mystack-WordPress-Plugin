...
(function($) {
    ...
    // Admin namespace
    window.mystackAdmin = window.mystackAdmin || {};
    
    // Configuration
    const config = {
        ajaxUrl: mystack_admin.ajax_url || ajaxurl,
        nonce: mystack_admin.nonce || '',
        strings: mystack_admin.strings || {}
    };
    ...
    const utils = {
        ...
        showLoading: function(element) {
            ...
            element.after('<span class="mystack-loading"></span>');
        },
        
        hideLoading: function(element) {
            element.prop('disabled', false);
            element.siblings('.mystack-loading').remove();
        },
    ...
    const SettingsPage = {
        ...
        bindEvents: function() {
            ...
            $(document).on('input', '#mystack_api_key', this.validateApiKeyInput);
            
            // API URL input validation
            $(document).on('input', '#mystack_api_url', this.validateApiUrlInput);
        },
                    ...
                    if (status === 'timeout') {
                        ...
                        if (typeof xhr.responseJSON.data === 'string') {
                            ...
                            if (xhr.responseJSON.data.debug) {
                                ...
                                console.log('mystack API Debug Info:', xhr.responseJSON.data.debug);
                            }
                        }
        ...
        loadModbots: function() {
            const select = $('#mystack_selected_modbot');
    ...
    const ModbotsPage = {
        ...
        loadModbots: function() {
            ...
            container.html('<div class="mystack-loading-container"><span class="mystack-loading"></span> Loading modbots...</div>');
            ...
            if (modbots.length === 0) {
                container.html(`
                    <div class="notice notice-info">
                        <p>No modbots found. <a href="https://app.mystack.ai/modbots" target="_blank">Create one in your mystack dashboard</a>.</p>
                    </div>
                `);
                return;
            }
            
            let html = '<div class="mystack-modbots-grid">';
            
            modbots.forEach(modbot => {
                html += `
                    <div class="mystack-modbot-card">
                        ...
                        <div class="modbot-actions">
                            ...
                            <button class="button button-primary copy-shortcode" data-shortcode="[mystack-modbot id=&quot;${modbot.id}&quot;]">
                                Copy Shortcode
                            </button>
                        </div>
                    </div>
        ...
        previewModbot: function(e) {
            ...
            const previewUrl = `https://app.mystack.ai/preview/modbot/${modbotId}`;
    ...
    const TicketFormsPage = {
        ...
        loadTicketForms: function() {
            ...
            container.html('<div class="mystack-loading-container"><span class="mystack-loading"></span> Loading ticket forms...</div>');
            
            $.ajax({
                url: config.ajaxUrl,
                type: 'POST',
                data: {
                    action: 'mystack_get_ticket_forms',
                    nonce: config.nonce
                },
                success: function(response) {
                    if (response.success && response.data) {
        ...
        renderTicketForms: function(forms) {
            ...
            if (forms.length === 0) {
                container.html(`
                    <div class="notice notice-info">
                        <p>No ticket forms found. <a href="https://app.mystack.ai/forms" target="_blank">Create one in your mystack dashboard</a>.</p>
                    </div>
                `);
                return;
            }
            
            let html = '<div class="mystack-forms-grid">';
            
            forms.forEach(form => {
                html += `
                    <div class="mystack-form-card">
                        ...
                        <div class="form-actions">
                            ...
                            <button class="button button-primary copy-shortcode" data-shortcode="[mystack-ticket-form id=&quot;${form.id}&quot;]">
                                Copy Shortcode
                            </button>
                        </div>
                    </div>
        ...
        previewForm: function(e) {
            ...
            const previewUrl = `https://app.mystack.ai/preview/form/${formId}`;
    ...
    $(document).ready(function() {
        ...
        if (currentPage && currentPage.includes('mystack-settings')) {
            SettingsPage.init();
        } else if (currentPage && currentPage.includes('mystack-modbots')) {
            ModbotsPage.init();
        } else if (currentPage && currentPage.includes('mystack-ticket-forms')) {
